...
## 🏗️ 1. Arhitektura Aplikacije i Baza Podataka
...
Ovo je srž projekta - simulacija izolacije sistema unutar kompanije.
...
*   Koristeći instrukcije `allow IP;` i `deny all;`, Firewall efikasno dropuje TCP mrežni saobraćaj svima koji nisu "Whitelisted", štiteći aplikaciju na nivou 4 odnosno 7 OSI modela pre nego što onlajn haker uopšte vidi `index.php`.

---

## 💻 3. Vodič za rad sa Bazom Podataka i SQL Upiti

Tabela se lako održava, ali se podaci cene najviše pomoću analitike (SQL upita).

### 🛠️ Kako izvršavati upite nad bazom?
1.  **Kroz Terminal (Ugrađeno):**
    Pokreni komandu iz terminala tamo gde je tvoj projekat da uđeš direktno u MySQL konzolu:
    ```bash
...
Ovim projektom se simulira kompleksna troslojna arhitektura koja se sreće u IT korporacijama širom sveta.
